...
import delimited "D:\MDS\Envio_ao_MDS\Dados\Etapa_2\junta_ali.csv"
...
merge m:1 v9001 using "D:\MDS\Envio_ao_MDS\Dados\Etapa_2\dicionario_alimentos.dta", nogen

* Unificar o código de local de compra POF (v9004) com o dicionário de classificação CNAE
* Substituir <caminho> pelo local desejado para salvar o arquivo
merge m:1 v9004 using "D:\MDS\Envio_ao_MDS\Dados\Etapa_2\dicionario_locais.dta", nogen

* Salvar a base de alimentação com os dicionários
* Substituir <caminho> pelo local desejado para salvar o arquivo
save "*D:\MDS\Envio_ao_MDS\Dados\Etapa_3\pof_compras.dta", replace

clear all
* Substituir <caminho> pelo local onde foi salvo o arquivo anterior.
use "D:\MDS\Envio_ao_MDS\Dados\Etapa_3\pof_compras.dta"

*Variável para número de itens de alimentação
gen contagem=1

*Somando contagem e valor por chave unica 
collapse(sum) contagem valor_mensal, by(uf estrato_pof tipo_situacao_reg cod_upa 
num_dom num_uc renda_total classe_prod cod_local nome_v9001 v9004 nome_local desc_local)
...
save "*D:\MDS\Envio_ao_MDS\Dados\Etapa_3\pof_compras_morador", replace

/*					JUNTANDO POF DATA COM MORADOR DATA
*/

clear all
cd "*D:\MDS\Envio_ao_MDS\Dados\Etapa_3\"
use morador_uc.dta
tostring uf estrato_pof tipo_situacao_reg cod_upa num_dom num_uc, replace
gen key_morador=uf+estrato_pof+tipo_situacao_reg+cod_upa+num_dom+num_uc
...
clear
use "*D:\MDS\Envio_ao_MDS\Dados\Etapa_3\pof_compras_morador.dta"
